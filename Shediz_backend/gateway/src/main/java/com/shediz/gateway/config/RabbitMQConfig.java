...
public class RabbitMQConfig
{
    ...
    {
        return BindingBuilder.bind(queue1).to(exchange).with(properties.getRouting1Key());
    }

    @Bean
    public Binding binding2(Queue queue2, DirectExchange exchange)
    {
        return BindingBuilder.bind(queue2).to(exchange).with(properties.getRouting2Key());
    }

    @Bean
    public Binding binding3(Queue queue3, DirectExchange exchange)
    {
        return BindingBuilder.bind(queue3).to(exchange).with(properties.getRouting3Key());
    }
